...
if ($con) {
	$x = 1;
	$stmt = $con->prepare("SELECT notif_id, client_id, case_type, case_details FROM notifications WHERE lawyer_id = ? AND accepted_status = 'not yet accepted'");
	$id = (int) $_SESSION['lawyer_id'];
	$stmt->bind_param('i', $id);
	$stmt->execute();
	$stmt->store_result();
	$stmt->bind_result($notif_id, $client_id, $case_type, $case_details);
    while ($stmt->fetch()) {
        ...
        $stmt2->bind_param("ssss", $id, $client_id, $case_type, $case_details);
        $stmt2->execute();
        $stmt2->store_result();
        $stmt2->bind_result($case_id);
        $stmt2->fetch();
    
        //$status = isset($_GET['status']) ? $_GET['status'] : ''; // Get the value of the "status" parameter from the URL
       // if ($status === 'rejected') {
