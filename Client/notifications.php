...
if ($con) {
	...
	$stmt->execute();
	$stmt->store_result();
	$stmt->bind_result($ctype, $cdetails, $acc_status);
	while ($stmt->fetch()) {
		if ($acc_status == 'accepted') {
			echo "
                            <div>
                                <p  style='font-size:20px;font-family:Times New Roman;color:green;'>Your case of {$ctype} with details {$cdetails} has been {$acc_status} by lawyer.</p>
                            </div>
                            ";
		}
			...
			echo "
                            <div>
                                <p style='font-size:20px;font-family:Times New Roman;color:red;'>
                                    Your case of {$ctype} with details {$cdetails} has been {$acc_status} by lawyer.
                                </p>
                            </div>
                            ";
		}
